...
sleeping, getting the time and setting interval timers with millisecond
granularity (well, _nanosecond_) etc while Lua is missing these.
...
Linux is _not a real-time operating system_ (`RTOS`). Although certain
features can be adjusted to this end (e.g. the nice value of a process,
tweaking the Linux Kernel task scheduler etc) they do not change this
...
are subject to the current system load, the OS scheduler etc; this
makes the accuracy rather volatile.

This is by way of explanation a long way of saying that `mockit`
expects input in _milliseconds_ and therefore allows _specification_
of sleep and timers with millisecond _granularity_ but it can't
_guarantee_ that these will be carried out with millisecond _precision_.
Timestamps and sleep do in fact seem to be both granular and precise
down to the millisecond (in C), but interval timers in particular will
suffer from slight (e.g. sometimes dozens of milliseconds or more)
of imprecision. In Lua, these may be worse as there are more levels of
...
expiry instances generate 'events' placed in an event queue. _Lua_
...
if they decide to only drain the queue every other minute, they will
...
This section contains various example code snippets to illustrate how
...
This is incorrect -- robust usage _should_ ensure the return values
are looked at so that error conditions are properly dealt with or
recovered from.

**FMI** on exactly what each function returns see the source code
`(src/mockit.c`); each function is amply commented so the information
is readily found.
...
`Mockit_mstimestamp()` aims to be one such convenience function.
```C
uint64_t msts = 0;
...
Below is another example of a timer augmented with user-allocated
...
Disarming an interval timer simply gets the timer to stop. It does
_not_ in and of itself release the resources associated with the timer
back to the system.
...
few nuances here:
 ...
 2. the timer object (`struct mockit`) and any user-allocated memory
    pointed to by the `ctx` field (if used) of the `mockit` struct must
    also be freed.
...
must therefore be joined at the end to release their thread resources
back to the system. Disarming an interval timer and then joining the
associated thread are both done at the same time via the common `destroy`
function used with interval timers.
...
timer object (`struct mockit`) - if dynamically allocated -  before
exitting the thread. The `ctx` field is not freed.
If the user uses this field (which is reserved for arbitrary use) to
point to some arbitrary user-allocated memory, they should also provide
a destructor that knows how to tear down the timer.
...
_and_ any memory pointed to by the `ctx` field of this struct as
 ...
 * the user _must_ call `Mockit_destroy()` on _interval_ timers to
   ...
   and should only call the former. `Mockit_destroy()` also waits to
...
interface to `mockit`. FMI about the return values of functions and such
see `src/luamockit.c`, which has ample comments, and for more examples
see `tests/tests.lua`.
...
**NOTES**
 ...
 - for the sake of simplicity the script mostly neglects to look at return
   values. For a robust usage, return values _should_ be looked at so
   as to catch and recover from/deal with error conditions.
...
primarily the lua callback associated with this event that must be
called back as a result of the timer having expired. The event gets
added to a global event queue in the `luamockit` library.

**At no point is LUA called back asynchronously.** This is what the
queue design is meant to accomplish.

The queue simply gets populated, from multiple threads (and
therefore enqueing and dequeing operations must be and are
protected by a mutex to ensure serialization).

The lua script itself must periodically call `luamockit.process_events()`
which will go through the event queue and dequeue each pending event
and 'handle' it. To handle an event means to call the Lua callback
associated with it, and then remove it from the event queue.

![Luamockit system design diagram](docs/luamockit.svg "Luamockit design diagram")

Therefore each call to `luamockit.process_events()` has a backlog that
it needs to clear. The more rarely `luamockit.process_events()` gets called,
the bigger the backlog and the less accurate the timers will seem.

That is to say, if the user registers a callback to be called back
every 3 seconds but the `luamockit.process_events()` gets called
every 10 seconds, then the callback will only get called every 10
seconds and every 10 seconds it will be called ~3 times in a row
(because the timer for the callback will have expired ~3 times),
placing ~3 events in the queue.

It's therefore important to call the `luamockit.process_events()`
function as often as possible : or, specifically, about as
frequently as the callback with the shortest interval timer.
...
The above points mean that a certain design naturally imposes itself
for a lua script using the interval timers feature of this library
(though this is by no means mandatory). The script will most often take
the form of an infinite loop that blocks with `luamockit.wait()` and
then calls `luamockit.process_events()` every time it unblocks.

This makes it most suited to scripts meant to be running as daemons.
This might not be as much of a problem as it sounds, since that's exactly
the context where one would typically even want to have callbacks called
asynchronously or at certain intervals: event loops dispatching events.
 ...
 * `make ctests`  /* run C tests  */
 * `make grind`   /* run C tests through valgrind (writes to valgrind.txt)*/
 * `make luatests`/* run lua tests */
