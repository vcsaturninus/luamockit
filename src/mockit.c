 ...
 */
inline bool Mockit_hasmod(uint8_t mark){
 ...
 */
static inline uint8_t Mockit_setmod(uint8_t mark){
 ...
 */
static inline uint8_t Mockit_setmfd(uint8_t mark){
 ...
 * mark must normally correspond to a mockit->mark__. 
 */
bool inline Mockit_ismfd(uint8_t mark){
...
static void *oneshot__(void *mockit){/* 
    ...
    if (mit->self_destr__ && mit->destructor){
        puts("in destructor, calling destructor");
        mit->destructor(mit->ctx);
...
int Mockit_oneoff(struct mockit *mit){
    ...
    assert(!pthread_detach(mit->thread_id__));
    return 0;
}
